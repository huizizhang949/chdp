...
#'
#' @description
#' This function plots probabilities against the covariate based on the MCMC samples from the post-processing step.
#'
#'
...
#' @param x a list of vectors. Each vector denotes the covariate in one dataset.
#' @param mfrow parameter used to cut the plot window into subpanels
#' @param data_names the labels for each dataset in the plot.
#' @param xlab one string. The label for x-axis in the plot.
...
#'
#' @examples
#' plot_probx(post_result = post_result, opt_cl = opt$opt_cl, x = list(t1, t2),
#'            mfrow = c(1,2), data_names = c('data1', 'data2'),
#'            xlab = 't', thinning = 5, truth = list(p_j1, p_j2))
plot_probx <- function(post_result, opt_cl, x, mfrow=c(2,2), data_names, xlab, thinning=NULL, truth=NULL,
  ...
  for(d in 1:D){
    ...
    for (j in Js) {
      ...
      plot(sort(x[[d]]),sort(x[[d]]),ylim=c(0,1),type='n',main=paste0(data_names[d],': cluster ',j),
           xlab=xlab, ylab='probability')

      for (i in ix) {
        lines(sort(x[[d]]),post_result$P_C_J_D_output[[d]][[i]][o,j],col='grey')
      }
      if(!is.null(truth)){
        lines(sort(x[[d]]),truth[[d]][o,j],col='red')
      }
    }
  }
