#' Perform MCMC for covariate-dependent hierarchical Dirichlet process to cluster across two time-series datasets based on VAR likelihood
#'
#' @description
...
#' @param empirical_z if \code{Z_fix} is not provided and \code{empirical_z=TRUE}, a simple Gaussian mixture model
...
pkernelHDP_mcmc <- function(Y, t, J = NULL, niter, burn_in = 1000, thinning = 1,
  ...
  # Prepare design matrix C_d[d] * (n_feature+1)
  X <- NULL
  for(d in 1:D){
  ...
  Y <- lapply(Y, function(l) {
    return(l[-1,])
  })
  t <- lapply(t, function(l) {
    return(l[-1])
  })
  ...
  if(is.null(Z_fix)){
    if(empirical_z==TRUE){
      ...
      mc_cluster <- Mclust(Y_all, G=J, modelNames = 'VVV')$classification
      Z_initial <- NULL
  ...
  lambda_J_D_initial <- do.call(cbind, lapply(1:D, function(d) {
    val <- rep(1/n_period[d]/pi, J)
    return(val)
  }))

  # Initials for r_J as an empirical mean from log(lambda_J_D_initial)
  r_J_initial <- apply(log(lambda_J_D_initial), 1, mean)

  # Initials for s^2
  s_2_initial <- mean(apply(log(lambda_J_D_initial), 1, var))

  # Initialize mu_J_D from the prior: Unif(-pi*lambda/2, pi*lambda/2)
  mu_J_D_initial <- do.call(cbind, lapply(1:D, function(d) {
    val <- runif(J, min=-pi*lambda_J_D_initial[,d]/2, max=pi*lambda_J_D_initial[,d]/2)
    return(val)
  }))
  ...
  sd_P_new <- 0.001
  mean_X_component_new <- log(matrix(P_new[1:(J-1)]/P_new[J], nrow = 1))
  ...
  for(iter in 2:(niter+1)){
    ...
    P_C_J_D_new <- lapply(1:D, function(d) {
      temp <- t(sapply(t[[d]],function(time) {
        Q_J_D_new[,d]*pkernel(time,mu_J_D_new[,d],lambda_J_D_new[,d],sigma_2_J_D_new[,d])/(
          sum(Q_J_D_new[,d]*pkernel(time,mu_J_D_new[,d],lambda_J_D_new[,d],sigma_2_J_D_new[,d])))
      }))
    })
